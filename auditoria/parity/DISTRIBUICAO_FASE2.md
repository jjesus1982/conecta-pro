...
- [x] T2 rh/turnover — troca turnover_audit_logs (log de atividade) → desligamentos reais (employees.data_demissao), mesma base do /turnover/dashboard do clássico (que não entrega predições por colaborador); Colaborador/Cargo/Data/Motivo, motivo nulo = 'aguardando dado' (data_gap honesto). Provado público (7 desligados). blue-green OK.
- [x] T2 gestao-de-pessoas/ged-envios — clássico usa document_kits (VAZIO) → redesign mantém ged_kit_access_logs (49 eventos reais, à frente do clássico); polido: ação→PT (Baixado/Visualizado), UUID de ator → rótulo do tipo (Interno/Cliente), nomes reais preservados. Provado público. blue-green OK.
- [x] T1 campo/checkin — CORRIGIDO fonte errada: mostrava visitas, o clássico mostra check-in/out de campo (gp_clock_punches). Agora bate (EDUARDO/MAIARA/FERNANDO com timestamps do dia). Novo campo.py override.
- [nota] T1 fiscal/certidoes: gap 14v10 = botões Sincronizar Todas/Emitir CNDs + abas de filtro (capacidade/UI, GATED) — o DADO (CNDs) já está fiel. Não é gap de dado.
- [x] T4 VARREDURA COMPLETA do cluster (oráculo): todos os gaps objetivos de DADO fechados (16 fixes). Restante confirmado SEM gap de dado: growth/banking/inter/boletos/cobrancas/fiscal/compras (só cabeçalho ou redesign mais rico); agendador/assistente/analytics (sem página no clássico / dados vazios honestos); vários "fora do menu". Itens que sobram = CAMADA DE ESCRITA (botões Analisar/Mascarar/Novo/Emitir/Sincronizar em migrador/mascaramento/leads/etc.) → dependem do frontend+gate do T1 (fundação #2), NÃO são dado. empresas/rentabilidade = deferido (margem sem tabela de custo real, não fabrico).
- [x] T2 portal-do-funcionario (5 telas) — ponto Tipo→PT (Entrada/Saída); beneficios/escalas/treinamentos status→PT (Cancelado/Agendado/Inativo/Concluído/Confirmado, espelha meu-espaco); dados-pessoais CPF formatado + exclui nome puramente numérico (registro duplicado junk). Provado público. blue-green OK. NOTA: existe registro duplicado do Jordan com nome='73068152291' (CPF no lugar do nome) — filtrado da exibição; limpeza de dado é decisão do Jordan.

## CAMADA DE ESCRITA (T4) — iniciada com gate OTP (Jordan aprovou "tudo com gate")
- [x] T4 financeiro/pagar-folha-pj — 1º BOTÃO money-out. Form gated (confirmar humano) + router /action/pagar-folha-pj que DELEGA ao serviço PROVADO pagamento_pj (OTP próprio: gera código→executar_lote). NUNCA auto-fire. Provado: form renderiza gated no menu; endpoint com mês inválido→400 antes de gerar OTP. Template p/ as demais. LIÇÃO: testar money-action chama serviço que EMAILA OTP real ao Jordan → daqui pra frente testo só por guard-inválido/inspeção, nunca disparando o OTP. commit 6a566d0a
- [x] T2 area-do-cliente (6 telas) — _bs só coloria (valor cru); novo _st traduz+colore. dashboard Segmento+Status PT; operacao/chamados/financeiro/analytics status PT (Concluída/Cancelada/Pendente/Paga); chamados Tipo+Severidade; kits UUID→'—' (client_id é ref. solta, não casa clients/condominiums) + status PT. Provado público. blue-green OK. NOTA: clients.health_score=0 p/ todos (métrica não computada — exibo honesto, não fabrico).
- [x] T4 crm/novo-lead + crm/nova-tarefa — botões de criação op_write (sem OTP/dinheiro), forms apontam pros endpoints provados /action/lead e /action/task. Escrita real reversível. curl público: forms gated no menu. commit 414ef8b5
- [x] T2 marketing/funil — Origem (referral→Indicação, whatsapp→WhatsApp) + Status (new→Novo, qualified→Qualificado) em PT, espelha constants/crm/leadStatus.ts; _bs deixava cru. Provado público. blue-green OK. NOTA: campanhas/lead-magnet/biblioteca/copywriter/estrategista/brand-voice = vazio-real (marketing_campaigns/assets/content_drafts com 0 linhas — sem dado, não fabricado).
- [x] T4 financeiro/pagar-diaristas — 2º botão money-out gated. Delega ao serviço provado pagamentos_diaristas (OTP próprio). Guard provado (data vazia→400 antes do OTP, sem e-mail). commit 2a37ef89. Inventário: crm já tinha 6 forms de escrita (não duplicar); empresas/migrador=agente lento 6s+inputs técnicos (baixo valor); falta LGPD (mascaramento/esquecimento).
- [x] T2 licitacoes/disputas — Modalidade PT (pregao_eletronico→Pregão Eletrônico) + Status PT (espelha statusLabel: Em Análise/Participar/Proposta Pronta/Em Disputa/GANHA/Perdida). Provado público. blue-green OK.
- [x] T2 licitacoes/resultados — RECUPERADA: filtro data_resultado IS NOT NULL deixava vazio; ampliado p/ status IN (won,lost) OR valor_homologado>0 → 5 resultados reais (TP-005 GANHA R$265k, PE-030 Perdida R$4,2M, etc). blue-green OK.
- [x] T2 equipamentos (4 telas: visao/patrimonio/comodatos/manutencoes) — VAZIO-REAL confirmado em AMBOS os lados: o clássico /equipment retorna {items:[],total:0} e stats zerados, lendo as MESMAS tabelas (equipments/equipment_comodatos/equipment_maintenances, todas 0 linhas). Sem gap de paridade, sem fantasma. Inventário de equipamentos não cadastrado ainda — não fabricado. Sem deploy (nenhuma mudança de código).
- [x] T4 empresas/nova-liminar — ESCRITA op_write (insert fiscal_liminares via gate). Fecha a escrita do empresas. Provado guard+insert+cleanup. commit d8a44cdd
- [~] T4 servicos/seguranca escrita — os botões restantes (Novo Agendamento/Contrato, Nova Solicitação esquecimento) são FK-pesados (diarist_id/client_id/titular_id) ou LGPD. FormScreen de texto não faz seletor de FK → precisa de frontend do T1. Seguranca destrutivo (mascaramento/esquecimento-execução) mantido FORA por decisão (irreversível em PII, exige supervisão do Jordan).
- [x] T2 automacoes (visao/workflows/execucoes) — VAZIO-REAL confirmado em AMBOS: clássico /crm/workflows retorna [] (200), lê crm_workflows/crm_workflow_runs (count(*) REAL=0; pg_stat n_live_tup estava stale). Sem gap, sem fantasma. Nenhum workflow CRM configurado ainda — não fabricado. Sem deploy. (billing_rules tem 14 linhas mas é domínio financeiro, fora do escopo do módulo automacoes=crm_workflows — igual ao clássico.)
- [x] T2 configuracoes — tenants/configuracoes-sistema: Plano (enterprise→Enterprise) + Status (active→Ativo); integracoes: log_type capitalizado (Webhook delivery) + nível PT (Info/Erro/Aviso). Provado público. blue-green OK. templates-notificacao (ai_email_templates=0) e feature-flags (feature_flags=0) = vazio-real; consultor (24 memórias IA reais) intacto.
- [x] T4 SWEEP PLAYWRIGHT (script próprio, 61 telas, clássico×redesign): 1 gap real fixado = financeiro/dashboard Faturamento por Cliente (nfse por tomador). Classificados como NÃO-gap: contabilidade=plano de contas template (tabela vazia, não fabrico); relatorios DRE=já vive no meu bi/dre (reorganização); KPIs de resumo (clientes/comissoes/oportunidades/propostas/contatos/esquecimento/pia-dpia/agendador)=no subtítulo (KPI-card em tela table precisa do ModuleView do T1); leads/precificacao/pagamentos-pj/fornecedores/nfse-entrada/contas-pagar-receber=falso-positivo (mesmo dado, formatação de linha); agentes/cfo/consultor=telas-IA custom; migrador/criptografia Analisar/Criptografar=botões (write layer). commit bbedcc83
- [x] T2 FEATURE turnover/motivo_desligamento — fecha o data_gap (10 desligados sem motivo, sem fonte p/ backfill). Endpoint POST /people-management/human-resources/turnover/registrar-motivo (valida vocabulário CLT 8 motivos, só escreve em desligado sem motivo, {ok,message}) + GET desligados-sem-motivo; form 'registrar-motivo-desligamento' na tela turnover (select desligados+motivo+obs). Provado: validações + happy-path rollback (zero fabricado) + endpoint público live. O RH informa o motivo REAL pela tela. blue-green OK.
- [x] T2 FIX kits/condomínio — REVIRAVOLTA: o client_id dos kits NÃO estava órfão; o FK aponta p/ ged_clients (não clients). Join estava na tabela errada. Corrigido: area-do-cliente/kits mostra o condomínio real + gestao-de-pessoas/ged-kits ganhou coluna Condomínio. 58/58 kits resolvem p/ 11 condomínios reais (IDEAL FLORES, PRIME ARENA, GELAIN...). Provado público. Sem re-ingestão, sem fabricar. RESOLVE o achado 'client_id solto'.
- [x] T2 FEATURE area-do-cliente/health_score — computado AO VIVO de sinais reais (aprovado Jordan): Pagamento 70% (pagas/(pagas+vencidas) via receivable_accounts por nome) + Contrato 30% (contracts.client_id ativo). Ocorrências dropadas (sem link a cliente). Rescalona pelos sinais; sem sinal → n/d. Cores clássico (≥70/≥40). Antes: coalesce(...,0) exibia '0.0' enganoso (NULL nos 20). Provado público (VILLA DOS PASSAROS 100, IDEAL FLORES 65, LARANJEIRAS 30). blue-green OK. Não persiste, não fabrica.

## AUDITORIA QA FINAL T2 (scanner automatizado 94 telas / 21k células)
- [x] Health HTTP 10/10 módulos = 200 (zero 500/schema drift).
- [x] Scanner red-flags: 16 flags → **13 bugs reais corrigidos** (enum cru/UUID/snake/formato em telas-BASE não sobrescritas) + 3 falsos-positivos ('Enterprise' = label correto). Todos provados no público.
